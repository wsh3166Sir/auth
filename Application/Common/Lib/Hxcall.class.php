<?php
 class Hxcall
 {
    ...
    private $app_key = 'sealyyg#gichy';
    private $client_id = 'YXA6ieSzYNVDEeWGAp-dsXmYwQ';
    private $client_secret = 'YXA6-RSqxC7HNcSKultc8INuQGFw0v0';
    private $url = "https://a1.easemob.com/sealyyg/gichy";
    /*
     * 获取APP管理员Token
     */
    function __construct()
    {
        ...
        $time = time();
        if(empty($resArr)){
            $resArr = json_decode($this->curl($url, $data), true);
            $resArr['additme'] = $time;
            M('kv')->where($where)->setField('value',json_encode($resArr));
        }else{
            if(($time - $resArr['addtime']) >=$resArr['expires_in']){
                $resArr = json_decode($this->curl($url, $data), true);
                $resArr['additme'] = $time;
                M('kv')->where($where)->setField('value',json_encode($resArr));
            }
