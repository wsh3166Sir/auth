...
var SuMarkdown=function(option){
   var load=function(mark){
       ...
       if(!option.clean){
           var suW = $(option.target).width();
           var suPreviewH = parseInt(option.height)-1;
           var textareaH = parseInt(option.height)-21;
           $('.suPreview',mark).css({
               "width":suW/2-21+'px',
               "float":"left",
               'overflow': 'auto',
               'padding': '0 10px',
               'height': suPreviewH+'px'
           });
           $('.suEditor',mark).css({
               "width":suW/2+'px',
               "float":"left",
      ...
      /* if(option.textHeight){
       ...
       // if(option.height) mark.css('height',option.height);

   };

    if(!option.target) option.target=$('.ui_suMarkdown');
